void readKinematics(const char* filename = "MCgen.dat")
{
  ...
  TH1F* hMassMeson = new TH1F("hMassMeson","Mass of all photons",2000,0.,2.);
  TH1F* hEnergyMeson = new TH1F("hEnergyMeson","Energy of all photons",10000,0.,1.);
  ...
  while(fgets(string,30000,file))
    {
      lineNumb++;
      if(string[0]=='#')continue; // comments
      if(string[0]=='/')continue; // comments
      if(string[0]=='\n')continue;
      if(strstr(string,"begin event")){
	
	if(eventCounter%100 == 0) cout<<"Read event # "<<eventCounter<<endl;
	fgets(string,30000,file);
	lineNumb++;
	sscanf(string,"%d %d %le %le %le\n", &eventNumb, &Ngamma, &eventVertex[0], &eventVertex[1], &eventVertex[2]);
	//cout<< "event number = " << eventNumb<< ", Ngamma = "<<Ngamma<<", X of vertex = "<< eventVertex[0]<< endl;
	fgets(string,30000,file);
	lineNumb++;
	...
	for(int i = 0; i<Ngamma;i++) {
	  ...
	  lineNumb++;
	  //cout<< string<<endl;
	  sscanf(string,"%le %le %le %le %le %le %le\n", &Pphotons[0][i], &Pphotons[1][i], &Pphotons[2][i], &Pphotons[3][i], &Pphotons[4][i], &Pphotons[5][i], &Pphotons[6][i]);
	...
	for(int i = 0; i<Ngamma;i++) {
	  Pgamma[i].SetXYZT(Pphotons[0][i],Pphotons[1][i],Pphotons[2][i],Pphotons[3][i]);
	  //Pgamma[i].Print();
	  Pmeson += Pgamma[i];
	}
	...
	cout<<"Broken format at line "<<lineNumb<<" : "<<endl<<string<<endl<<"Abort!"<<endl;
	return;
      }
      eventCounter++;
    }
