...
for HYCONDITION in empty s4t2sa #empty
do
...
#	for TGTPRFX in al35mm cu7mm sn5mm pb3mm c78mm be79mm ch80mm # al35mm cu7mm sn5mm pb3mm
	for TGTPRFX in be79mm #pb3mm al35mm cu7mm sn5mm c78mm be79mm
	do
	    #for per in 2012-04 2012-11 2015-03 2014-11 2013-03
	    for per in 2015-03
	    do
		ieff=0
		for eff in 0.91 0.92 0.93 0.94 0.95 0.96 0.97 0.98 0.99
		do
		    ieff=$[ $ieff + 1 ]
		    unset DELTA_SA
		    if [ $per = 2015-03 ] ; then 
		    ...
		    export PERIOD=$per
		    MESON=${MES}
		    UNIC_CODE=$[ $cond*1000000+ $n*100000 + 90000 + $ieff*1000 ]
		    export TGT_PRFX=$TGTPRFX
		    export EFFICIENCY_SA=$eff
		    #export PRODUCTION_NAME=${TGT_PRFX}_${MES}_${HYCONDITION}
		    export PRODUCTION_NAME=${TGT_PRFX}_${MES}_${HYCONDITION}_effSa${eff}
		    export HYMC_CONFIG_DEFINED=yes
		    echo " "
		    echo  MESON =                $MESON
