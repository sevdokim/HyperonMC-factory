...
c
 ...
 5697    call abtocds(Pa,Pb(5),Pc,Pd,T)
         if (T.gt.0) then 
            ...
            if(ncalls_reaction.gt.100) then
...
C      R2 = REGRNDM(0)
c      T = R1*2.
c      if((R2*500.).gt.Double_T_distr_f2(T)) go to 434
c      T = -T
         b = 5.1                ! from fit of Be79mm data by exp convoluted with effXres 
         Random = REGRNDM(0)
         T = (1./b)*dlog(Random)
         ncalls=ncalls+1
