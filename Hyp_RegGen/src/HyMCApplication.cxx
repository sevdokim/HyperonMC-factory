...
HyMCApplication::HyMCApplication()
    : TVirtualMCApplication(), fStack(0), fDetConstruction() {
  /// Default constructor
  EvntNumb = 0;
  evWr = 0;
  fStack = new HyMCStack(1000000);
...
void HyMCApplication::RunMC(Int_t nofEvents) {
  ...
  fPbeam.SetXYZT(0., 0., fMomentum,
                 sqrt(fMomentum * fMomentum + 0.139570 * 0.139570));
...
void HyMCApplication::GeneratePrimaries() {
  ...
  Int_t pdg =
      22; // 22 - photon; kProton; kPositron; Pi+ 211; Pi0 111; Elektron - 11;
  ...
  //------Fortran subroutine returns array of 8-momentums (px, py, pz, E, M, x,
  // y, z) of 10 photons
  //------XYZ are non-zeros for K0s production only-----------

  if (EvntNumb ==
      0) { // firts call; it's time to set up interaction probabilities
  ...
  if (rndmNumb < TargProb[0]) {
    Ntarget = 1;
  } else if (rndmNumb < TargProb[0] + TargProb[1]) {
    Ntarget = 2;
  } else
    Ntarget = 3;
  if (fExTarget > 0)
  ...
  //  if(EvntNumb == 0) {
  //    Ngamma = gRandom->Integer(4294967296); //randomize numerical continua in
  //    eventgen Ntarget=fExTarget; // if ExTarget, ExReson, ExChanel >= 0 then
  //    these values will be used as stearing parameters Nreson=fExReson;
  //    //otherwise those from generator source code will be used
  //    Nchanel=fExChanel;
  ...
  //  }
  //  if(fDebug) cout<<endl<<counter<<" times tried to generate photons"<<" for
  //  event number "<<EvntNumb<<endl; if(counter >= 10000){
  //    fStack->PushTrack(toBeDone, -1, 0, 0., 0., 1., 1., 0., 0., 0., 0., polx,
  //    poly, polz, kPPrimary, ntr, 1., 0); cout<<"Failed to generate
  //    photons"<<endl; cout<<"\nGeantino added"<<endl;
  //  }
  ...
  Double_t pout[500][10]; // remember inversed order of indices in 2D array in
                          // fortran
  Float_t FlBeam = fMomentum;
  Int_t nTries = 0;
  Int_t iError = 0, jRec = 0, istOut = 0;
  Int_t fortran_seed = gRandom->Integer(
      iseed);          // pass the seed into the RegGen generator ;kondr v.1.04
  if (EvntNumb == 0) { // randomize RegGen
    nTries = gRandom->Integer(
        4294967295); // randomize numerical continua in eventgen
  }
  ...
  while (nTries < 1000 && iError) { // in case of generator error try to
                                    // generate until number of tries < 1000
    pout[0][0] = fControl;
    istOut = fExChanel;
    reggen_event_(&FlBeam, &nTries, &iError, &fortran_seed, &jRec, &istOut,
                  &pout[0][0]);
  ...
  if (fDebug == 2) { // print generated reaction number
    cout << "RegGen: I generated 1 event with reaction number " << jRec
         << " and I tried " << nTries << " times. seed is " << fortran_seed
         << endl;
  }
  ...
  for (int i = 0; i < nFinal; ++i) {
    ...
    if (fDebug == 2) { // print generated event
      cout << "Particle " << i << " with PDG code " << pout[i][8]
  ...
  for (int i = 0; i < nFinal; i++) {
    ...
    int tmpI = indices[i];
    double tmpD = angles[i];
    angles[i] = maxA;
    indices[i] = indices[maxI];
    angles[maxI] = tmpD;
    indices[maxI] = tmpI;
    if (fDebug)
      cout << i << "-th particle in stack has angle " << angles[i] << endl;
  }
  ...
  // if they produce hits in Sa then such event is rejected by trigger
  // conditions and tracking is stopped to save time
  ...
  for (int i = 0; i < nFinal; ++i) {
    ...
    if (!isPushed[i])
      continue; // charged particle will be added later
    if (pdg == 100) {
      toBeDone = 0;
    } // do not track
    else
    ...
    // fprintf(fMCgen_dat,"%.16E %.16E %.16E %.16E %.16E %.16E
    // %.16E\n",px,py,pz,e,x,y,z);
  ...
  for (int i = 0; i < nFinal; ++i) {
    if (isPushed[i])
      continue; // skip particle if it is already added to the stack
    // particle properties
    px = pout[indices[i]][0];
    py = pout[indices[i]][1];
    pz = pout[indices[i]][2];
  ...
  //  // fprintf(fMCgen_dat,"/# This is generator output \n");
  //  // fprintf(fMCgen_dat,"/# 1st string: 'begin event' word \n");
  //  // fprintf(fMCgen_dat,"/# 2nd string: Nevent Ngamma Xvertex Yvertex
  //  Zvertex \n");
  //  // fprintf(fMCgen_dat,"/# 3rd string: PbeamX PbeamY PbeamZ PbeamE \n");
  //  // fprintf(fMCgen_dat,"/# 4th string: PgammaX PgammaY PgammaZ PgammaE
  //  Xgamma Ygamma Zgamma of 1st photon\n");
  //  // fprintf(fMCgen_dat,"/# ...........\n");
  //  // fprintf(fMCgen_dat,"/# (3+Ngamma)th string: PgammaX PgammaY PgammaZ
  //  PgammaE Xgamma Ygamma Zgamma of (Ngamma)th photon\n");
  //  // fprintf(fMCgen_dat,"/# means comment )\n");

  //  fprintf(fMCgen_dat,"begin event\n");
  //  fprintf(fMCgen_dat,"%d %d %.16E %.16E %.16E\n",EvntNumb,Ngamma,vertex[0],
  //  vertex[1], vertex[2]); fprintf(fMCgen_dat,"%.16E %.16E %.16E %.16E\n",
  ...
  //     if (!isnan(e)){fStack->PushTrack(toBeDone, -1, pdg, px, py, pz, e,
  //     vertex[0]+x, vertex[1]+y, vertex[2]+z, 0., polx, poly, polz, kPPrimary,
  //     ntr, 1., 0);//adding photon to stack }else
  // 	fStack->PushTrack(toBeDone, -1, 0, 0., 0., 1., 1., 0., 0., 0., 0., polx,
  // poly, polz, kPPrimary, ntr, 1., 0);
  //     if(fDebug){
  // 	//gammas printing
  ...
  /*
    ...
    if (!isnan(e))fStack->PushTrack(toBeDone, -1, pdg, px, py, pz, e,
    vertex[0]+x, vertex[1]+y, vertex[2]+z, 0., polx, poly, polz, kPPrimary,
    ntr, 1., 0);//adding missing mass products to stack
    }
    if(fDebug){
    cout<<"\n=====missing mass======"<<endl;
    for(int iPart=0;iPart<NPDGcodes;iPart++){
  ...
  */
  ...
  // backward going pion to simulate beam energy deposition. no. let's assume
  // that we always have some signal in s4.
  //  pdg=211;//pi+
  //  px=-pBeam.X();
  //  py=-pBeam.Y();
  //  pz=-pBeam.Z();
  ...
  // if(Ngamma>0){
  //                    #gammas   #missing mass products  #beam particle #counts
  //                    from N-1 to 0
...
void HyMCApplication::Stepping() {
  ...
  if ((strcmp(gMC->CurrentVolName(), "SM_SENS_GLAS") == 0)) { /*Center*/
    ...
    energy_dep[current_primary_number][Ix - 1 + (Iy - 1) * 8 + 576] +=
        gMC->Edep();
  }
...
void HyMCApplication::FinishEvent() {
  ...
  for (i = 0; i < fLaNx; i++) {
    for (Int_t j = 0; j < fLaNy; j++) {
      EnTotal += LaEnDep[i][j];
      //      e[j+i*LaNy]=float(LaEnDep[i][j]); // Eqvivalence: i = ix+(iy-1)*24
      //      for large glasses
    }
  }
...
int HyMCApplication::GetSeed() { return iseed; }

void HyMCApplication::SetServer(std::string _server) { g_server = _server; }

void HyMCApplication::SetPort(int _port) { g_port = _port; }

void HyMCApplication::SetConnection() { SetConnection(_NF_HOST_, _NF_PORT_); }
...
void HyMCApplication::MessageCycle(bool sime_global) {
  ...
  std::string
      message; // = make_message(123, "message", "here is the message ge !");
  message = make_message(GetSeed(), "alive", "justmc");
  printf("message to send: '%s'\n", message.c_str());
  n = write(sockfd, message.c_str(), message.size());
  //  message = make_message(GetSeed(), "justmc", "");    printf("message to
  //  send: '%s'\n",message.c_str());  n =
  ...
  while (true) {
    ...
    // &serv_addr,sizeof(serv_addr)) < 0) { printf("ERROR connecting\n"); return
    // ; }
    //     printf("zz '%s'\n",message.c_str());
    ...
    n = write(sockfd, message.c_str(), message.size());
    //    message = make_message(GetSeed(), make_head("chat"), EvntNumb);    n =
    //    write(sockfd,message.c_str(),message.size());
...
void HyMCApplication::MessageCycle2(bool sime_global) {
  ...
  //   cout << "ee-";
  //   cout .flush();
  //  if (mi_main != NULL){ cout << mi_main << "hmmmm-" << (*mi_main); cout
  //  .flush();
...
//*****************************************************************************************//
// the following functions withdraw some information from the message or create
// message of necessary format:
...
std::string HyMCApplication::make_block(std::string block) {
  ...
  if (block.size() % _BLOCK_LENGTH_ == 0)
    return block;
  if (block.size() + _BLOCK_LENGTH_ - (block.size() % _BLOCK_LENGTH_) <
      _NF_MAX_BUFFER_LENGTH_)
    block.resize(block.size() + _BLOCK_LENGTH_ -
                     (block.size() % _BLOCK_LENGTH_),
                 _EMPTY_SYMBOL_);
  else
    block.resize(_NF_MAX_BUFFER_LENGTH_, _EMPTY_SYMBOL_);
  return block; // "[____][____][____][__" -> "[____][____][____][____]"  fill
                // the rest of the block at the end with the "_EMPTY_SYMBOL_"
                // (don't forget to compare it with the max_buffer_length)
}

/*
std::string HyMCApplication::append_block(std::string &message, std::string
block)
{
...
std::string HyMCApplication::append_block(std::string message,
                                          std::string block) {
  message.append(make_block(block));
  return message;
}
...
std::string HyMCApplication::make_message(int int_client_id, std::string head,
                                          std::string message = "") {
  std::stringstream ss;
...
std::string HyMCApplication::get_client_id(std::string message) {
  try {
    ...
    client_id = message.substr(
        0,
        std::min(_ID_LENGTH_, (int)(client_id.find_first_of(_EMPTY_SYMBOL_))));
    return client_id;
...
int HyMCApplication::get_int_client_id(std::string message) {
  return std::atoi(
      get_client_id(message).c_str()); // atoi transfers c-string (char) into
                                       // int. (atof-into float)
}
...
std::string HyMCApplication::get_head(std::string message) {
  try {
    ...
    head = head.substr(
        0, std::min(_HEAD_LENGTH_, (int)(head.find_first_of(_EMPTY_SYMBOL_))));
    return head;
  } catch (std::exception &e) {
...
std::string HyMCApplication::get_block(
    std::string message, int block_number = 2) // from 2 ; id -> 0 ; head -> 1.
{
  try {
    std::string block = message.substr(_ID_LENGTH_ + _HEAD_LENGTH_ +
                                           _BLOCK_LENGTH_ * (block_number - 2),
                                       _BLOCK_LENGTH_);
    block =
        block.substr(0, std::min(_BLOCK_LENGTH_,
                                 (int)(block.find_first_of(_EMPTY_SYMBOL_))));
    return block;
  } catch (std::exception &e) {
...
std::string HyMCApplication::get_chat_message(std::string message) {
  try {
    return message.substr(_ID_LENGTH_ + _HEAD_LENGTH_,
                          message.size() - (_ID_LENGTH_ + _HEAD_LENGTH_));
    ;
  } catch (std::exception &e) {
    std::cerr << "Exception in get_head: " << e.what() << "\n";
