...
HyDetectorConstruction::HyDetectorConstruction()
    : TObject(), fGeom(), fImedGlass(0), fImedPlastic(0), fImedBe(0),
      fImedAir(0), fImedAl(0) {
...
void HyDetectorConstruction::ConstructVolumes() {
  ...
  //     posZ = -fGeom.GetMembraneDist() -
  //     i*(fGeom.GetMembraneBlockLength())/(fGeom.GetNofMembranes());
...
void HyDetectorConstruction::CalcInteractProbabilities(float *ProbArray) {
  ...
  ProbArray[0] = float(1 - TMath::Exp(-fGeom.GetS4Thickness() * PolystDens /
                                      PolystDensNuclIntrLngth));
  if (fDebug)
    cout << "Interaction probability of S4 = " << ProbArray[0] << endl;
  ;

  if (fTrgtType == "Be")
    ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness() * BeDens /
                                        BeDensNuclIntrLngth));

  if (fTrgtType == "Al")
    ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness() * AlDens /
                                        AlDensNuclIntrLngth));

  if (fTrgtType == "Cu")
    ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness() * CuDens /
                                        CuDensNuclIntrLngth));

  if (fTrgtType == "Sn")
    ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness() * SnDens /
                                        SnDensNuclIntrLngth));

  if (fTrgtType == "Pb")
    ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness() * PbDens /
                                        PbDensNuclIntrLngth));

  if (fTrgtType == "Carbon")
    ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness() * CDens /
                                        CDensNuclIntrLngth));

  if (fDebug)
    cout << "Interaction probability of target = " << ProbArray[1] << endl;

  // ProbArray[2] = float(1 -
  // TMath::Exp(-fGeom.GetMembraneWidth()*fGeom.GetNofMembranes()*MylarDens/MylarDensNuclIntrLngth));
  ...
  if (fDebug)
    cout << "Interaction probability of Sa = " << ProbArray[2] << endl;
}
//__________________________________________________________________________________________________
void HyDetectorConstruction::GambleDecayVertex(Int_t NofTarget,
                                               Double_t *vertex) {
  vertex[0] = 0.;
  vertex[1] = 0.;
  // XY beam profile
  ...
  case 1: // S4
    vertex[2] = fGeom.GetS4Dist() - fGeom.GetS4Thickness() / 2. +
                fRandGen->Uniform(fGeom.GetS4Thickness());
    break;
  case 2: // Target
    vertex[2] = fGeom.GetTargetDist() - fGeom.GetTargetThickness() / 2. +
                fRandGen->Uniform(fGeom.GetTargetThickness());
    break;
  case 4: // membranes
    vertex[2] = -fGeom.GetMembraneDist() -
                ((int)(fRandGen->Uniform(1.) * fGeom.GetNofMembranes())) *
                    (fGeom.GetMembraneBlockLength()) /
                    (fGeom.GetNofMembranes());
    break;
  case 3: // Sa
    vertex[2] = fGeom.GetSaDist() - fGeom.GetSaThickness() / 2. +
                fRandGen->Uniform(fGeom.GetSaThickness());
    break;
  default:
    vertex[2] = -450.;
  }
...
Int_t HyDetectorConstruction::GetTargetMaterial() { return fTargetMaterial; }
