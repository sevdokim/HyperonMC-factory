...
HyDetectorConstruction::HyDetectorConstruction()
  : TObject(),
    fGeom(),
    fImedGlass(0),
    fImedPlastic(0),
...
//_____________________________________________________________________________
void HyDetectorConstruction::SetGeometry(HyGeoParameters newgeom)
{
  fGeom=newgeom;
}
void HyDetectorConstruction::SetTargetMaterial(int mat)
{
  if(mat==2){fSettedTargetMaterial=mat;return;}
  if(mat==3){fSettedTargetMaterial=mat;return;}
  if(mat==7){fSettedTargetMaterial=mat;return;}
  if(mat==8){fSettedTargetMaterial=mat;return;}
  if(mat==9){fSettedTargetMaterial=mat;return;}
  if(mat==10){fSettedTargetMaterial=mat;return;}
  if(mat==11){fSettedTargetMaterial=mat;return;}
  fSettedTargetMaterial=0;
}
//_____________________________________________________________________________
HyGeoParameters HyDetectorConstruction::GetGeometry()
{
  return fGeom;
}
//_____________________________________________________________________________
...
{
  new TGeoManager("Hyperon_geometry", "Hyperon VMC example geometry");
  ...
  //____Be____
  TGeoMaterial* matBe = new TGeoMaterial("Be", a = 9.01, z = 4., density = 1.848); 
  //____Al____
  TGeoMaterial* matAl = new TGeoMaterial("Al", a = 26.98, z = 13., density = 2.699);
  //____Mylar____
  ...
  //____Cu____
  TGeoMaterial* matCu = new TGeoMaterial("Cu", a = 63.546, z = 29., density = 8.96);
  //____Sn____
  TGeoMaterial* matSn = new TGeoMaterial("Sn", a = 118.71, z = 50., density = 7.31);
  //____Pb____
  TGeoMaterial* matPb = new TGeoMaterial("Pb", a = 207.19 , z = 82., density = 11.35);
  // Tracking media
...
{ 
  ...
  float BeamPos[3]={0.,0.,0.}; //Beam position relativ. LGD2 center (read from calibr.cards)
  char buf[500];
  ...
  if(!CalibrCards) {
    ...
    while(fgets(buf,500,CalibrCards)){
      if(buf[0]=='C'&&buf[1]=='*')continue;//comment simbols "C*"
      if(strstr(buf,"TARG")!=0)sscanf(buf,"TARG %f %f %f",&BeamPos[0],&BeamPos[1],&BeamPos[2]);
    }
  }
  ...
  if(fSettedTargetMaterial==0) {
    ...
    default : 
      fTrgtType = "Be";
      //fGeom.SetTargetThickness(5.5);
      targMaterial = fImedBe;
      cerr << "Typed material is incorrect. Target isn't changed. Press any key and then enter to continue.";
      cin >> tmp;

    }
  }
  else{targMaterial=fSettedTargetMaterial;}
  ...
  fGeom.SetTargetDist(fGeom.GetLGD2Dist()+fGeom.GetMatrixDist()-distance/10.);
  ...
  if(env_val!=NULL){
    cout<<"S4 thickness is modified by setting the environment variable THICKNESS_S4 = ";
    Double_t val;
    sscanf(env_val,"%lf",&val);
    cout<< val << " mm." << endl;
  ...
  if(env_val!=NULL){
    cout<<"Target is shifted by setting the environment variable DELTA_TARGET = ";
    Double_t val;
    sscanf(env_val,"%lf",&val);
    cout<< val << " mm." << endl;
  ...
  cout<<"Target Z_position ="<<fGeom.GetTargetDist()<<" cm."<<endl<<endl;
  
  cout<<"Sa thickness = "<<fGeom.GetSaThickness()<<" cm."<<endl;
  cout<<"Sa Z_position ="<<fGeom.GetSaDist()<<" cm."<<endl<<endl;
  
  cout<<"Z_position of LGD2 surface = "<<fGeom.GetLGD2Dist()+fGeom.GetMatrixDist()<<" cm."<<endl;
  ...
  Double_t LGD2[3];
  LGD2[0]=(fGeom.GetLaGlassSize())*(fGeom.GetLaNx()/2.)+fGeom.GetMatrixDist();
  LGD2[1]=(fGeom.GetLaGlassSize())*(fGeom.GetLaNy()/2.)+fGeom.GetMatrixDist();
  ...
  Double_t Glass[3];
  Glass[0] = fGeom.GetLaGlassSize()*fGeom.GetLaNx()/4.-fGeom.GetSmGlassSize()*fGeom.GetSmNx()/4.;
  ...
  Glass[1] = fGeom.GetLaGlassSize()*fGeom.GetLaNy()/4.-fGeom.GetSmGlassSize()*fGeom.GetSmNy()/4.;
  Glass[2] = fGeom.GetGlassLength()/2.;
  ...
  posX = (-1)*(fGeom.GetLaGlassSize())*(fGeom.GetLaNx()/4.+fGeom.GetSmNx()/8.); 
  gGeoManager->Node("GLASLEFT", 1, "LGD2IN", posX, 0, 0, 0, kTRUE, ubuf);
  posX = (-1)*posX;
  gGeoManager->Node("GLASLEFT", 2, "LGD2IN", posX, 0, 0, 0, kTRUE, ubuf);

  posY = (fGeom.GetLaGlassSize())*(fGeom.GetLaNy()/4.+fGeom.GetSmNy()/8.);
  gGeoManager->Node("GLASTOP", 1, "LGD2IN", 0, posY, 0, 0, kTRUE, ubuf);
  posY = (-1)*posY;
  gGeoManager->Node("GLASTOP", 2, "LGD2IN", 0, posY, 0, 0, kTRUE, ubuf);

  //Divide on X axis
    gGeoManager->Division("GLASLEFT_XDIV","GLASLEFT", 1, 0, 0, fGeom.GetLaGlassSize(), 0, "S");
    gGeoManager->Division("GLASTOP_XDIV","GLASTOP", 1, 0, 0, fGeom.GetLaGlassSize(), 0, "S");
    gGeoManager->Division("GLACENTR_XDIV","GLACENTR", 1, 0, 0, fGeom.GetSmGlassSize(), 0, "S");
  
  //Create Mylar shell
    gGeoManager->Division("GLASLEFT_DIVIDED","GLASLEFT_XDIV", 2, 0, 0, fGeom.GetLaGlassSize(), fImedMylar," S");
    gGeoManager->Division("GLASTOP_DIVIDED","GLASTOP_XDIV", 2, 0, 0, fGeom.GetLaGlassSize(), fImedMylar, "S");
    gGeoManager->Division("GLACENTR_DIVIDED","GLACENTR_XDIV", 2, 0, 0, fGeom.GetSmGlassSize(), fImedMylar, "S");
  ...
  gGeoManager->Node("LA_SENS_GLAS", 1,"GLASLEFT_DIVIDED", 0, 0, 0, 0, kTRUE, ubuf);
  gGeoManager->Node("LA_SENS_GLAS", 2,"GLASTOP_DIVIDED", 0, 0, 0, 0, kTRUE, ubuf);
  Glass[0] = fGeom.GetSmGlassSize()/2.-0.01;
  Glass[1] = Glass[0];
  Glass[2] = fGeom.GetGlassLength()/2.-0.01;
  gGeoManager->Volume("SM_SENS_GLAS", "BOX", fImedGlass, Glass, 3);
  gGeoManager->Node("SM_SENS_GLAS", 1,"GLACENTR_DIVIDED", 0, 0, 0, 0, kTRUE, ubuf);
  ...
  // Double_t Memb[3];
  // Memb[0] = fGeom.GetMembraneSize()/2.;
  // Memb[1] = Memb[0];
  // Memb[2] = fGeom.GetMembraneWidth()/2.;
  // gGeoManager->Volume("MEMBRANE", "BOX", fImedMylar, Memb, 3);
  // //posZ = (-1)*(fGeom.GetMembraneDist());
  // for(Int_t i=0;i<fGeom.GetNofMembranes();i++)
  //   {
  //     gGeoManager->Node("MEMBRANE",i+1,"WRLD",0 ,0, posZ, 0, kTRUE, ubuf);
  //     posZ = -fGeom.GetMembraneDist() - i*(fGeom.GetMembraneBlockLength())/(fGeom.GetNofMembranes());
...
void HyDetectorConstruction::CalcInteractProbabilities(float* ProbArray)
{
  float CDens  = 1.742;
  float BeDens = 1.848;
  float AlDens = 2.7;
  ...
  ProbArray[0] = float(1 - TMath::Exp(-fGeom.GetS4Thickness()*PolystDens/PolystDensNuclIntrLngth));
  if(fDebug) cout<<"Interaction probability of S4 = " <<ProbArray[0]<<endl;;

    if(fTrgtType == "Be" )
      ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness()*BeDens/BeDensNuclIntrLngth));
     
    if(fTrgtType == "Al" )
      ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness()*AlDens/AlDensNuclIntrLngth));
     
    if(fTrgtType == "Cu" )
      ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness()*CuDens/CuDensNuclIntrLngth));
     
    if(fTrgtType == "Sn" )
      ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness()*SnDens/SnDensNuclIntrLngth));
      
    if(fTrgtType == "Pb" )
      ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness()*PbDens/PbDensNuclIntrLngth));
      
    if(fTrgtType == "Carbon" )
      ProbArray[1] = float(1 - TMath::Exp(-fGeom.GetTargetThickness()*CDens/CDensNuclIntrLngth));

    if(fDebug)cout<<"Interaction probability of target = " <<ProbArray[1]<<endl;
    
    //ProbArray[2] = float(1 - TMath::Exp(-fGeom.GetMembraneWidth()*fGeom.GetNofMembranes()*MylarDens/MylarDensNuclIntrLngth));
    
    //cout<<"Interaction probability of membranes = " <<ProbArray[2]<<endl;

    ProbArray[2] = float(1 - TMath::Exp(-fGeom.GetSaThickness()*PolystDens/PolystDensNuclIntrLngth));//assuming that all pi+A interactions gives some signal in Sa due to pi+ and are suppressed by trigger
    
    if(fDebug)cout<<"Interaction probability of Sa = " <<ProbArray[2]<<endl;
    
}
//__________________________________________________________________________________________________
void HyDetectorConstruction::GambleDecayVertex(Int_t NofTarget, Double_t* vertex)
{
  vertex[0] = 0.;
  vertex[1] = 0.;
  //XY beam profile
  double r=10.,phi;
  while(r>6.){
  r=sqrt(-2*log(fRandGen->Uniform(0.,1.)));
  //sigma xy
  r=r*2.5;//cm
  }
  phi=fRandGen->Uniform(0.,1.)*2*3.1415926;
  vertex[0] = r*cos(phi);
  vertex[1] = r*sin(phi);
  switch (NofTarget){
  case 1://S4
    vertex[2] = fGeom.GetS4Dist()-fGeom.GetS4Thickness()/2. + fRandGen->Uniform(fGeom.GetS4Thickness());
    break; 
  case 2://Target
    vertex[2] =  fGeom.GetTargetDist()-fGeom.GetTargetThickness()/2. + fRandGen->Uniform(fGeom.GetTargetThickness());
    break;
  case 4://membranes
    vertex[2] = -fGeom.GetMembraneDist() - ((int)(fRandGen->Uniform(1.)*fGeom.GetNofMembranes()))*(fGeom.GetMembraneBlockLength())/(fGeom.GetNofMembranes());
    break;
  case 3://Sa
    vertex[2] =  fGeom.GetSaDist()-fGeom.GetSaThickness()/2. + fRandGen->Uniform(fGeom.GetSaThickness());
    break;
  default:
    vertex[2] = -450.;
  }
...
Int_t HyDetectorConstruction::GetTargetMaterial()
{
