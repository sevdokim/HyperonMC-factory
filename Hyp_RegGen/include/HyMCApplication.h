...
extern "C" {
void reggen_event_(float *BeamMomentum, int *Ievent, int *Ierror,
                   int *fortran_seed, int *Jrec, int *IstOut, double *Pout);
//  void reggen_event_(float *BeamMomentum, int *i1, int *i2, int *Ievent, int
//  *Ierror,int *fortran_seed, int *Jrec, int *IstOut, double *Pout);
// void decay_missingmass_(int *iResonanceType, float *rProtonsToNucleons,
// double* Pmismas, int* NPDGcodes, int* MPDGcode, double* Pproducts);
}
...
public:
  ...
  void SetMCgendat(const char *filename) { fMCgendat = filename; }
  ...
  void SetExTarget(int hh) { fExTarget = hh; }
...
private:
  ...
  const char *fMCResults; ///< filename to save MC results
  FILE *fMC_results;      ///< file to save MC results
  const char *fHistosRoot;       ///< filename to save histos
  FILE *fMCgen_dat;              ///< file to save MC T spectrum
  const char *fMCgendat;         ///< filename to save T spectrum
  ...
  double fControl; ///< parameter to control event gen(see describtion in
                   ///< hyp_carlo.f)
  int fExReson;  ///< exclusive resonance  (steering parameter for event gen, if
                 ///< <0 then ignored)
  int fExChanel; ///< exclusive chanel     (steering parameter for event gen, if
                 ///< <0 then ignored)
  int fExTarget; ///< exclusive target     (steering parameter for event gen, if
                 ///< <0 then ignored)
