...
C
     ...
     +  7.,  1.,  2.,  8.,   15.,  -7.,    1.,    1.,     2.,     3.,  !  pi+n -> Empty   + X incl Carlo  
     +  8.,  1.,  2.,  8.,   15.,  -7.,    1.,    1.,     2.,     3.,  !  pi+n -> Empty   + X incl Carlo  
     +  9.,  1.,  2.,  8.,   15.,  -7.,    1.,    1.,     2.,     3.,  !  pi+n -> Empty   + X incl Carlo   
...
C      
      if (Jspn.eq.1) then
         ...
         call BW_rand_f2(Amas, Rwid, BW_mass)
      else
...
C      
      if (N_Reg.eq.Ireg2) then
                          IRegg = Ireg2
                          alpha0= alpha(1,IRegg)
                          alpha1= alpha(2,IRegg)
                          go to 10
                   endif           
      enddo
C
   10 T = Alog(rand())/(2.*alpha1*Dlog(S))     
      return
      end      
C      
...
C     generate resonance in mass limits (135MeV : 2000MeV)
 ...
 10   rnd_BW=135.+1865.*rand()
      BW=rnd_BW*Rwid/((rnd_BW**2-Rmas**2)**2+(rnd_BW*Rwid)**2)
      BW_max=1./(Rmas*Rwid)
      if(BW.lt.BW_max*rand())goto 10
      return
      end
c
...
C-      write(*,*) 'BW_f2:', BW_class, BW_gams, X_mas
      if (BW_class*rand().gt.BW_gams) go to 10  
      rnd_BW   =   X_mas
      return
      end
C
...
C-    data    ExTarget, ExReson, ExChanel  / 3, 4, 1/ !  The main parameters for f2 exclusive channel 
      ...
      data Mtarget,Mgamma,Mreson  / 3, 9, 7 /         !  3 targets, 9 photons maximum, 7 resonanses (yet) 
...
C                                             Energy of the virtual photon
   10 Omega =-Omax*alog(RAND())
      if (Omega.lt.Omin.or.Omega.gt.Omax)     go to 10
      if (Wmax*RAND().gt.alog(Omax/Omega)) go to 10
C      	                              
   20 qt =-Otmx*alog(RAND())          !    Trannsverse momentum
      if (qt.gt.Otmx)    go to 20
      Omg  = Omega/gamma
      Wqmx = 1./(16.*Omg)                !    Omgmx = sqrt(3)*Omg
      if (Wqmx*RAND().gt.qt**3/(Omg**2+qt**2)**2) go to 20
C  
      t     =-Omega**2-qt**2
      phi   = pi2*RAND()
      Pt(1) = qt*cos(phi)
...
C
  ...
  111 if (RAND().lt.(0.493544008)) then 
         Mean = 2.9667
         s2 = 0.40686
      else
         Mean = 2.1106
