...
C.........................................................................................
C        version 2.15 2pi0 parametrisation from GAMS paper is added to hyp_carlo
...
C
      ...
      if (Nreson.eq.10) then                        !  Resonance 10 - a2(1320) (former rho0(770))
C     rho0(770)
c$$$C         if (Pgamma(1,0).gt.0.) WidOmg = Pgamma(1,0)*1000. !  read width from control parameter
c$$$         call BW_rand_omg(AmRho0,WidRho0 ,rnd_BW)     !  GAMS BW for omg(782)  
c$$$         Pc(5) = 0.001*rnd_BW                       !  MeV => GeV   
c$$$C    
c$$$         call reaction(3, T, Pd(5))                 !  Just like omega 
c$$$         call abtocds(Pa,Pb(5),Pc,Pd,T)      
c$$$         if (T.gt.0) go to 1000 
c$$$         if (Nchanel.eq.1) then                     !  Excl. channel = rho0(770) -> pi0Y -> 3Y
c$$$          P1(5) = Ampi0
c$$$          call decays(Pc, P1, Pgamma(1,1))
c$$$          call decays(P1, Pgamma(1,2), Pgamma(1,3))
c$$$          Ngamma = 3
c$$$         endif
C     a2(1320)
         L=2
...
C!!!- Ngamma = 0
      ...
      END       
C
      subroutine S_pi0pi0(X_mas)
      common/minmax/ X_min,X_max,A_max
      real regrndm
      data X_min,X_max,A_max / 0.3, 2.3, 19000. /
C
 10    X_mas = X_min+(X_max-X_min)*REGRNDM(0)
C      
      Wait  = s_fit_100gev(X_mas)
      if (A_max*REGRNDM(0).gt.Wait) go to 10
C      
      RETURN
      END
C
      function s_fit_100gev(X)
C
C     Fit of the S-wave 2pi0 data at 38 + 100 GeV
C     Eur.Phys.J. A3, 361-371 (1998), Fig.10
C
      common/pawpar/ par(12)
      common/PurPur/ Ampi,Amrs(4),Wdrs(4),Pur(12)
      Logical Lstart
      data    Lstart / .false. /
C-    data Nres,Amrs / 4, 0.960, 1.580, 1.315, 2.010 /
      data Nres,Amrs / 4, 0.965, 1.590, 1.315, 2.075 /   ! Xi2=0.8864, my fit
C-    data      Wdrs /    0.070, 0.280, 0.190, 0.240 /
      data      Wdrs /    0.090, 0.290, 0.200, 0.240 /   ! Xi2=0.8864, my fit
C
C     vec/cea par(12) r 719.18 0.623898 1.79851 -0.159459 0.0726618 108.917 -20.9783 28.8362 46.0504 -15.0503 3.53881 11.5122 Xi2=0.8864
C
      if(Lstart) go to 10
      Ampi  = 0.1349766
      Pur( 1) = 719.18
      ...
      Lstart=.true. 
C      
 10    Ampl = Pur(1)*(X-2.*Ampi)**abs(Pur(2))*exp(-Pur(3)*X-Pur(4)*X*X)
C
      SRe = Ampl
      SIm = 0.
      do j=1,Nres
      ResR=ResRe(X,Amrs(j),Wdrs(j))
      ResI=ResIm(X,Amrs(j),Wdrs(j))
C      
      SRe = SRe + Pur(2*j+3)*ResR - Pur(2*j+4)*ResI
      SIm = SIm + Pur(2*j+3)*ResI + Pur(2*j+4)*ResR
      enddo
C
      s_fit_100gev = SRe*SRe + SIm*SIm 
      return
      end
C
      function ResRe(XX,Am,Wd)
C     Real BW      
C      
      Ampi = 0.1349766
      q  = sqrt(0.25*XX**2 - Ampi**2)
      q0 = sqrt(0.25*Am**2 - Ampi**2)
      G  = Wd*q/q0
      ResRe=(XX/q)*(Am*G)*(Am**2-XX**2)/((Am**2-XX**2)**2+(G*Am)**2)              
      return
      end    
C      
      function ResIm(XX,Am,Wd)
C     Image BW
C      
      Ampi = 0.1349766
      q  = sqrt(0.25*XX**2 - Ampi**2)
      q0 = sqrt(0.25*Am**2 - Ampi**2)
      G  = Wd*q/q0
      ResIm=(XX/q)*(Am*G)*(Am*G)/((Am**2-XX**2)**2+(G*Am)**2)              
      return
      end  
C
C      
