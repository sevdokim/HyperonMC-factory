  ...
  if (first_time) {
    ...
    if (ecormatrix != NULL) {
      ...
      for (iStep = 0; iStep <= 100000; iStep++) {
        ...
        e_cur = ecor_goal;
        // if(iStep%200==0)cout<<"goal en ="<<ecor_goal<<" and current
        // en="<<e_cur<<endl;
        ...
        // if(iStep%200==0)cout<<"e_corr = " <<e_corr[iStep]<< "  for e = " <<
        // e[iStep]<< " and iStep = "<<iStep<<endl;
      }
    }
  }
...
double draw_unecorr(double *x, double *par) { return une_corr_new(x[0]); }
  ...
  while (fscanf(input, "%c", &string[0]) != EOF) {
    ...
    if (old_version) { // old format version
      ...
      if (string[0] == '#') {
        for (i = 0; i < PhotonNumber; i++)
          for (j = 0; j < 640; j++) {
            ...
            // if(i+1==PhotonNumber&&e[j]>0.05)e[j]=une_corr_new(e[j]); //uncor
            // for every cell - old way e[j]+=energy_dep[i][j]*1.159; //uncor
            // for every cell - old way
            e[j] +=
                energy_dep[i][j] * 1.159 *
                une_corr_new(initial_photon_energy[i]) /
                initial_photon_energy[i]; // unecor for every photon - new way
            energy_dep[i][j] = 0;
          }
        writehyp_(e, RunNumber, ieve);
        for (j = 0; j < 640; j++)
      ...
      while (fin >= 0) {
        ...
        //}
        j = fin;
        if (fscanf(input, "%lf %d ", &energy_dep[PhotonNumber][fin], &fin) ==
            EOF)
          break;
