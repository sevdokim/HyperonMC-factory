...
{
  if (buf == NULL) {
    cerr << "Cannot read int_from_string_after_symbol() from NULL string!!!"<<endl;
    return 0;
  }
  int i = 0,val;
  ...
  if(buf[i] == '\n' || buf[i] == '\0' )  {
    cerr << "Cannot read int_from_string_after_symbol(): symbol "<<symbol<<" not found!!!"<<endl;
    return 0;
  }
  //cout<<&buf[i+1]<<endl;
...
static void readConnectionTable()
{
   ...
   cout << "Address format : "<<Cher[0]<<" ; Adr 1 :"<<Cher[1]<<" ;Adr 2 :"<<Cher[2]<<" ;Adr 3 :"<<Cher[3]<<endl;
     ...
     while ((fgets(buf, 1024, fp))) {
	  ...
	  for(i=0;i<7;i++) {
	    for(j=0;j<96;j++) {
	      ...
	      if( ig>0 ) {chn = big ? ig : 576 + ig;
		if(fHadr[chn-1]>0) printf("double geo connection to cell %d\n",chn);
   ...
   if (!fp) {
	...
	char str[8];
	 for (i = 0; i<576; i++)
        	{
		  if((i)%12==0&&i!=0)fgets(str,2,fp);
		  fgets(str,8,fp);
		  sscanf(str,"%f", &fCalibr[i]);
		  printf("%7.3f ",fCalibr[i]);
		  if((i+1)%12==0){printf("\n");}
		  //fscanf(fp, "%f", &fCalibr[i]);//doesn't work if you have something like 123.456123.456 in coef_old.dat  
		}
         for (i = 0; i<64; i++)
                {
		  if((i)%8==0)fgets(str,2,fp);
		  fgets(str,8,fp);
...
static void newBlock() // in fact new record, not used
{
   ...
   gBuf[0] = 1048579; // the type of data (taken from the real data) data type = 3
   gBuf[1] = fBlockNumber;
   gBuf[2] = 1133544553; // time (some value taken from real data)
...
// Evd 17.04.2019
      float A=2,B=3,C=1,D=3,E=0,sigma;// sigma/E = sqrt(A^2/E + B^2/E^2 + C^2)
      sigma=0.01*e[i]*sqrt(A*A/e[i] + B*B/e[i]/e[i] + C*C + D*D*e[i]+ E*E*e[i]*e[i]);
...
//   gTotalLen = 0;
   ...
   //gBuf[0] = 1048577; // taken from 2015-11 cu data (datatype = 1) ! not working
    gBuf[0] = 1048579; // the type of data (taken from the real data) data type = 3
   gBuf[1] = fBlockNumber;
   ...
   gBuf[4] = 0;   // separator
   //gBuf[5] = gGroupLen+4;   // length of the first group of data + 4 (words) worked ok
   gBuf[5] = gGroupLen+3;
   gBuf[6] = 0;   // group id
   gBuf[7] = 71;   // device id
}
