...
C-      write(*,*) 'BW_f2:', BW_class, BW_gams, X_mas
...
C-    data    ExTarget, ExReson, ExChanel  / 3, 4, 1/ !  The main parameters for f2 exclusive channel 
      ...
      data Mtarget,Mgamma,Mreson  / 3, 9, 9 /         !  3 targets, 9 photons maximum, 9 resonanses (yet) 
...
C
      data Mchanel  /     1,   3,   1,   1,   1,  1,  1, 1, 1, 0 /   !  Decay cahannels
      data Pchanel  /  1.00, 9*0.0,                              !  pi0 /pi0->2Y/
     2                39.31, 32.57, 0.027,  7*0.0,               !  eta /eta->2Y, eta->3pi0->6Y, eta->2Ypi0->4Y/
     3                1.000, 9*0.0,                              !  w /w->pi0Y->3Y/
     4                30.69, 9*0.0,                              !  K0 /K0s->2pi0->4Y/
     5                28.27, 9*0.0,                              !  f2 /f2->2pi0->4Y/
...
c30      write(*,*) 'hyp_event: I did ', n30calls, 'calls of mark (30)'
      R1 = REGRNDM(0)
...
CSdv-    width_f2 = Pgamma(1,0) ! read width from control variable (see run_g3_control.C).
CSdv-    if(width_f2.le.0.0) width_f2 = 186.7
         call BW_rand_f2(1275.5, 186.7,rnd_BW)      !  GAMS BW for f2(1275)
CSdv-    call BW_rand_f2(1275.5, width_f2,rnd_BW)   !  GAMS BW for f2(1275)
...
c
 ...
 5697    call abtocds(Pa,Pb(5),Pc,Pd,T)
         if (T.gt.0) then 
            ...
            if(ncalls_reaction.gt.3) then
               write(*,*)'cannot generate Missing mass for mass = ', Pc(5)
               goto 5696
            endif
            go to 5697                                                                        
         endif                                                                                
...
C                                                                                             
 ...
 854     R1=REGRNDM(0)                                                                        
         R2=REGRNDM(0)                                                                        
         FF=Double_Mass_distr_2pi0(R1*2100.)                                                  
C        write(*,*) 'Mass distr = ',FF                                                       
         if((R2*800.).gt.FF) go to 854                                                        
         Pc(5) = R1*2.1                                                                       
         Pc(5) = 0.270 + 1.830*REGRNDM(0)           !  ===> Uniform distribution
         P1(5) = Ampi0                                                                        
         P2(5) = Ampi0                                                                        
         call reaction(Nreson, T, Pd(5))
...
C
      Random = REGRNDM(0)
C-    if (Random.lt.0.42) then
      if (Random.lt.0.34) then
