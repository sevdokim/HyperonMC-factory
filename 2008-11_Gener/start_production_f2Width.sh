...
do
    ...
    for MES in f2 #omg #eta f2 2pi0 K0 # f0
    do 	
	case "$MES" in
            "pi0")     n=1 ;;
            "eta")     n=2 ;;
...
#	for TGTPRFX in be79mm
	do
	    for width in {160..220..5} #c is control parameter for omg width 
	    do
		...
		export PRODUCTION_NAME=$PERIOD_PRFX$TGT_PRFX  # production name
		export PRODUCTION_NAME=${PRODUCTION_NAME}_${MES}_width${width}MeV_$HYCONDITION
		export HYMC_CONFIG_DEFINED=yes
