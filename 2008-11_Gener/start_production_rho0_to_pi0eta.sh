...
do
    ...
    for MES in rho0 #f2 #omg #eta f2 2pi0 K0 # f0
    do 	
	case "$MES" in
            "pi0")     n=1 ;;
            "eta")     n=2 ;;
...
#	for TGTPRFX in al35mm cu7mm sn5mm pb3mm c78mm be79mm ch80mm # al35mm cu7mm sn5mm pb3mm
	...
	do
		...
		*)          export THICKNESS_S4=6.000  ;; #default value (6mm) 
	    ...
	    export PRODUCTION_NAME=$PERIOD_PRFX$TGT_PRFX  # production name
	    export EXCHANEL=4 # rho0 -> pi0eta -> 4gam
	    export PRODUCTION_NAME=${PRODUCTION_NAME}_${MES}_to_pi0eta_${HYCONDITION}
	    export HYMC_CONFIG_DEFINED=yes
	    export CONTROL=0.77 # control=0.77 makes omg meson to be rho0
	    echo " "
	    echo  MESON =                $MESON
