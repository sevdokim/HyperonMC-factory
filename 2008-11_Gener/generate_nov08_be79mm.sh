...
then
    ...
    export NTHREADS=8                             # total number of production threads
    ...
    export PRODUCTION_NAME=${PRODUCTION_NAME}_f2_missMassProton
...
export REGGEN_CARDS=$MCDIR/${PERIOD}_Gener/RegGen_nov08_be79mm_f2hypCarlo.cards        #name of RegGen.cards
...
do
  cd $MCDIR
  export SEED=$[ $i+$TARGET*1000+50000 ]
  echo "$i "'SEED='$SEED;
