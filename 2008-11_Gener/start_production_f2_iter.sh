...
export NTHREADS=110                             # total number of production threads   
...
#	for TGTPRFX in be79mm
	do
		...
		*)          export THICKNESS_S4=6.000  ;; #default value (6mm) 
	    ...
	    export f2_mass=$(cat f2_measurements_iter${iter}.txt | grep $TGTPRFX | awk -F' ' '{ print $5}')
	    export f2_width=$(cat f2_measurements_iter${iter}.txt | grep $TGTPRFX | awk -F' ' '{ print $8}')
	    export TGT_PRFX=$TGTPRFX
	    export PRODUCTION_NAME=$PERIOD_PRFX$TGT_PRFX  # production name
	    export PRODUCTION_NAME=${PRODUCTION_NAME}_f2_iter${iter}_${HYCONDITION}
	    export HYMC_CONFIG_DEFINED=yes
	    export CONTROL=11101
	    echo " "
	    echo  MESON =                $MESON
	    ...
	    echo 'f2_mass             =' $f2_mass
	    echo 'f2_width            =' $f2_width
