...
#	for TGTPRFX in al35mm cu7mm sn5mm pb3mm c78mm be79mm # al35mm cu7mm sn5mm pb3mm
	for TGTPRFX in c78mm
	do
	    for channel in 1001 1002 #1003 1004 #channel is control parameter for eta exclusive channel selection 
	    do
		MESON=${MES}
		UNIC_CODE=$[ $cond*1000000+ $n*100000 + $channel*10000 ]
		export TGT_PRFX=$TGTPRFX
		export PRODUCTION_NAME=$PERIOD_PRFX$TGT_PRFX  # production name
		export PRODUCTION_NAME=${PRODUCTION_NAME}_${MESON}
		case "$channel" in
		    "1001")  export PRODUCTION_NAME=${PRODUCTION_NAME}2gamma ;;
		    "1002")  export PRODUCTION_NAME=${PRODUCTION_NAME}3pi0 ;;
		    #"1003")  export PRODUCTION_NAME=${PRODUCTION_NAME}2pi0 ;;
		    #"1004")  export PRODUCTION_NAME=${PRODUCTION_NAME}etapi0 ;;
		    *)       export PRODUCTION_NAME=${PRODUCTION_NAME} ;;
		esac
		export PRODUCTION_NAME=${PRODUCTION_NAME}_$HYCONDITION
		export HYMC_CONFIG_DEFINED=yes
		export CONTROL=$channel
		echo " "
		echo  MESON =                $MESON
