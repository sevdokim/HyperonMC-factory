...
do
    ...
    for MES in R #omg #eta f2 2pi0 K0 # f0
    do 	
	case "$MES" in
            "pi0")     n=1 ;;
            "eta")     n=2 ;;
	...
	export MESON=$MES
#	for TGTPRFX in cu7mm sn5mm pb3mm c78mm be79mm ch80mm # al35mm cu7mm sn5mm pb3mm
	...
	do
		...
		*)          export THICKNESS_S4=6.000  ;; #default value (6mm) 
		...
		export PRODUCTION_NAME=$PERIOD_PRFX$TGT_PRFX  # production name
		export PRODUCTION_NAME=${PRODUCTION_NAME}_${MES}_M${mass}MeV_${HYCONDITION}
		export HYMC_CONFIG_DEFINED=yes
		export CONTROL=$mass
		echo " "
		echo  MESON =                $MESON
